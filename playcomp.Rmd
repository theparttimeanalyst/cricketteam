...
bat_kpi = vital_bat %>% left_join(bat_sum_in, by = c("bat", "comp_fin4")) %>%
                            left_join(bat_sum_ga, by = c("bat", "comp_fin4")) %>%
                              ...
                              select(bat, comp_fin4, wpa10per, eawpaper, restwpaper, ppper, mid_per, death_per, bpi_per, rpi_per) %>%
                                            pivot_longer(cols = 4:11, names_to = "kpi", values_to = "percent") 
...
ggplot(data = play_sum3, aes(x = stat, y = percentile, label= percentile, fill = position)) +
  # add the bar/pizza slices that are colored
  geom_bar(data = play_sum3, width = 1,
           color = "oldlace",
           stat = "identity") +
  ...
  geom_bar(aes(y=100, fill=position), stat="identity", width=1, alpha=0.5) +

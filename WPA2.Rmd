...
t20blast_statsv2 = t20blast_stats %>% bind_rows(average_bowl) %>%
  ...
  player = c("NR Welch", "avplay_BA", "WL Madsen", "JL du Plooy", "Anuj Dal","BD Guest", "MH McKiernan", "ZJ Chappell", "MRJ Watt", "GLS Scrimshaw", "avplay_BW")


  team2_bowl = tibble(player) %>%
                            left_join(t20blast_stats, by = "player") %>%
                              mutate(bowlrank = rank(desc(bpi)))  %>%
                                filter(bowlrank < 6) %>%
  ...
  player = c("NR Welch", "SG Budinger", "RK Patel", "CN Ackermann", "PWA Mulder","AM Lilley", "Rehan Ahmed", "HJ Swindells", "CF Parkinson", "WS Davis", "Naseem Shah")


  team1_bowl = tibble(player) %>%
                            left_join(t20blast_stats, by = "player") %>%
                              mutate(bowlrank = rank(desc(bpi)))  %>%
                                filter(bowlrank < 6) %>%
                                  mutate(bowlr = rank(desc(wpa6))) %>%
...
team1  = team1_bowl %>% bind_cols(team1_bat) %>%
                            bind_cols(team2_batop) %>%
                                bind_cols(team2_bowlop)  %>%
                                  mutate(team = "foxes")


team2 = team2_bowl %>% bind_cols(team2_bat) %>%
                                    bind_cols(team1_batop) %>%
                                      bind_cols(team1_bowlop) %>%
                                        mutate(team = "falcons")



match = team1 %>% bind_rows(team2)



match_pred = predict(fin_fitv4, match, type = "prob")


match2 = match %>% bind_cols(match_pred)
...
# dur lanc
  ...
  player1 = c("G Clark", "AZ Lees", "MA Jones", "OG Robinson", "AJ Turner","avplay_BA", "L Doneathy", "L Trevaskis", "BA Raine", "NA Sowter", "WD Parnell")



  play5 = c("PD Salt", "JC Buttler", "SJ Croft", "LWP Wells", "DJ Mitchell","LS Livingstone", "C de Grandhomme", "TW Hartley", "L Wood", "TE Bailey", "MW Parkinson")

  
  
  match_pred = function(x,y){
...
match_durlanc = match_pred(play5, player1)
...
```{r}
  ...
  player1 = c("EJ Byrom", "SA Northeast", "KS Carlson", "CA Ingram", "CB Cooke","HW Podmore", "WT Root", "DA Douthwaite", "P Sisodiya", "P Hatzoglou", "JP McIlroy")



  play5 = c("TS Muyeye", "DJ Bell-Drummond", "JL Denly", "SW Billings", "JM Cox","JA Leaning", "GF Linde", "G Stewart", "JDM Evison", "Qais Ahmad", "MG Hogan")
...
match_gk = match %>% bind_cols(match_pred)
...
````{r}
  ...
  player1 = c("AM Rossington", "FIN Khushi", "DW Lawrence", "PI Walter", "RJ Das","DR Sams", "T Westley", "SR Harmer", "MJJ Critchley", "S Snater", "SJ Cook")



  play5 = c("BR McDermott", "JM Vince", "TJ Prest", "JJ Weatherley", "RA Whiteley","JK Fuller", "TE Albert", "LA Dawson", "NT Ellis", "MS Crane", "CP Wood")
...
match_eh = match %>% bind_cols(match_pred)
...
```{r}
  ...
  player1 = c("T Banton", "WCF Smeed", "T Kohler-Cadmore", "TB Abell", "TA Lammonby","L Gregory", "BGF Green", "RE van der Merwe", "JH Davey", "MJ Henry", "PM Siddle")



  play5 = c("SS Eskinazi", "JB Cracknell", "MDE Holden", "PJ Malan", "LBK Hollman","RF Higgins", "JLB Davies", "MK Andersson", "N Fernandes", "TG Helm", "BC Cullen")
...
match_sm = match %>% bind_cols(match_pred)
...
```{r}
  ...
  player1 = c("R Vasconcelos", "CA Lynn", "JJ Cobb", "DJ Willey", "SA Zaib","LD McManus", "TAI Taylor", "GG White", "AJ Tye", "JJG Sales", "FJ Heldreich")



  play5 = c("NR Welch", "LJ Hill", "RK Patel", "CN Ackermann", "Naseem Shah","LPJ Kimber", "Rehan Ahmed", "HJ Swindells", "CF Parkinson", "WS Davis", "Naveen-ul-Haq")
...
match_nl = match %>% bind_cols(match_pred)
...
```{r}
  ...
  player1 = c("G Roelofsen", "CDJ Dent", "Miles Hammond", "OJ Price", "JMR Taylor","BG Charlesworth", "James Bracey", "DJ Lamb", "M de Lange", "TMJ Smith", "DA Payne")



  play5 = c("WG Jacks", "LJ Evans", "SM Curran", "Tk Curran", "SP Narine","JL Smith", "SA Abbott", "J Overton", "CJ Jordan", "AAP Atkinson", "TE Lawes")
...
match_gs = match %>% bind_cols(match_pred)
...
```{r}
  ...
  player1 = c("MM Ali", "AL Davies", "GJ Maxwell", "SR Hain", "CG Benjamin","DR Mousley", "EG Barnard", "DR Briggs", "HJH Brookes", "JB Lintott", "CN Miles")



  play5 = c("JM Clarke", "AD Hales", "C Munro", "M Montgomery", "TJ Moores","LW James", "SJ Mullaney", "SR Patel", "Shaheen Shah Afridi", "M Carter", "C McKerr")
...
match_BN = match %>% bind_cols(match_pred)
 ...
 team2_bowl = tibble(play5) %>%
                            ...
                            left_join(t20blast_stats, by = "player") %>%
                              mutate(bowlrank = rank(desc(bpi)))  %>%
                                filter(bowlrank < 6) %>%
                                  mutate(bowlr = rank(desc(wpa6))) %>%
                                    select(player, wpa6, bowlr) %>%
                                        




 team2_bowl = tibble(player1) %>%
                                rename(player = play5) %>%
                            left_join(t20blast_stats, by = "player") %>%
                              mutate(bowlrank = rank(desc(bpi)))  %>%
                                filter(bowlrank < 6) %>%
...
```{r}
  ...
  player1 = c("BR McDermott", "JM Vince", "AHT Donald", "JJ Weatherley", "RA Whiteley","JK Fuller", "TE Albert", "LA Dawson", "NT Ellis", "MS Crane", "CP Wood")



  play5 = c("TGR Clark", "RS Bopara", "TP Alsop", "Shadab Khan", "MGK Burgess","JM Coles", "GHS Garton", "FJ Hudson-Prentice", "OJ Carter", "TS Mills", "HT Crocombe")
...
match_hs = match %>% bind_cols(match_pred)
...
```{r}
  ...
  player1 = c("A Lyth", "DJ Malan", "avplay_BA", "ML Revis", "Shan Masood","JA Thompson", "D Wiese", "BWM Mike", "JA Tattersall", "DM Bess", "JA Chohan")



  play5 = c("LM Reece", "Haider Ali", "WL Madsen", "JL du Plooy", "TA Wood","MH McKiernan", "BD Guest", "ZJ Chappell", "MRJ Watt", "GLS Scrimshaw", "avplay_BW")
...
match_yd = match %>% bind_cols(match_pred)
...
york = c("A Lyth", "DJ Malan", "avplay_BA", "ML Revis", "Shan Masood","JA Thompson", "D Wiese", "BWM Mike", "JA Tattersall", "DM Bess", "JA Chohan")
...
Middlesex = c("SS Eskinazi", "JB Cracknell", "MDE Holden", "PJ Malan", "LBK Hollman","RF Higgins", "JLB Davies", "MK Andersson", "N Fernandes", "TG Helm", "BC Cullen")
durham = c("G Clark", "AZ Lees", "MA Jones", "OG Robinson", "AJ Turner","avplay_BA", "L Doneathy", "L Trevaskis", "BA Raine", "NA Sowter", "WD Parnell")
lancs= c("PD Salt", "JC Buttler", "SJ Croft", "LWP Wells", "DJ Mitchell","LS Livingstone", "C de Grandhomme", "TW Hartley", "L Wood", "TE Bailey", "MW Parkinson")
foxes = c("NR Welch", "LJ Hill", "RK Patel", "CN Ackermann", "Naseem Shah","LPJ Kimber", "Rehan Ahmed", "HJ Swindells", "CF Parkinson", "WS Davis", "Naveen-ul-Haq")
sussex = c("TGR Clark", "RS Bopara", "TP Alsop", "Shadab Khan", "MGK Burgess","JM Coles", "GHS Garton", "FJ Hudson-Prentice", "OJ Carter", "TS Mills", "HT Crocombe")
glamor = c("EJ Byrom", "SA Northeast", "KS Carlson", "CA Ingram", "CB Cooke","HW Podmore", "WT Root", "DA Douthwaite", "P Sisodiya", "P Hatzoglou", "JP McIlroy")
somers = c("T Banton", "WCF Smeed", "T Kohler-Cadmore", "TB Abell", "TA Lammonby","L Gregory", "BGF Green", "RE van der Merwe", "JH Davey", "MJ Henry", "PM Siddle")
essex = c("AM Rossington", "FIN Khushi", "DW Lawrence", "PI Walter", "RJ Das","DR Sams", "T Westley", "SR Harmer", "MJJ Critchley", "S Snater", "SJ Cook")
kent = c("TS Muyeye", "DJ Bell-Drummond", "JL Denly", "SW Billings", "JM Cox","JA Leaning", "GF Linde", "G Stewart", "JDM Evison", "Qais Ahmad", "MG Hogan")
surrey = c("WG Jacks", "LJ Evans", "SM Curran", "Tk Curran", "SP Narine","JL Smith", "SA Abbott", "J Overton", "CJ Jordan", "AAP Atkinson", "TE Lawes")
northam = c("R Vasconcelos", "CA Lynn", "JJ Cobb", "DJ Willey", "SA Zaib","LD McManus", "TAI Taylor", "GG White", "AJ Tye", "JJG Sales", "FJ Heldreich")
...
notts = c("JM Clarke", "AD Hales", "C Munro", "M Montgomery", "TJ Moores","LW James", "SJ Mullaney", "SR Patel", "Shaheen Shah Afridi", "M Carter", "C McKerr")
...
match_sum = team_bat(york,derby, "yrks")  %>%
                     bind_rows(team_bat(glouces, Middlesex, "glou")) %>%
                               bind_rows(team_bat(durham, foxes, "durh")) %>%
                            bind_rows(team_bat(sussex, glamor, "sus")) %>%
                              bind_rows(team_bat(somers, essex, "somer")) %>%
                                  bind_rows(team_bat(kent, surrey, "ken")) %>%
                                    bind_rows(team_bat(worces, northam, "worc")) %>%
                                        bind_rows(team_bat(notts, lancs, "nott"))
...
# Combine common operations into functions
# This function creates a tibble for the players and joins with t20blast_stats
create_team_tibble <- function(player, team_name) {
  tibble(player) %>%
    left_join(t20blast_stats, by = "player") %>%
    mutate(team = team_name)
}

# This function calculates ratings for batting and bowling
calc_rating <- function(team_df, metric, rank_limit, min_metric_limit = NULL) {
  team_df %>%
    group_by(team) %>%
    filter(if (!is.null(min_metric_limit)) bb > min_metric_limit else TRUE) %>%
    mutate(rank = rank(desc(metric))) %>%
    filter(rank < rank_limit) %>%
    ungroup() %>%
    group_by(team) %>%
    summarise(rating = mean(metric))
}

# Creating team tibbles
hawks = create_team_tibble(c("JM Vince", "BR McDermott", "TJ Prest", "RA Whiteley", 
                            "TE Albert","LA Dawson", "NT Ellis", "CP Wood", 
                            "MS Crane", "SW Currie", "JJ Weatherley"), "hawks")
somerset = create_team_tibble(c("T Banton", "WCF Smeed", "T Kohler-Cadmore", 
                                "TB Abell", "TA Lammonby", "L Gregory", "BGF Green", 
                                "RE van der Merwe", "C Overton", "MJ Henry", "PM Siddle"), "somerset")

# Combine into a match
match4 = bind_rows(hawks, somerset)

# Calculate ratings for batting and bowling
match4_bat = calc_rating(match4, wpa10, 7)
match4_bowl = calc_rating(match4, wpa6, 6, 40)

# Add the opposition rating
match4_bat = match4_bat %>% 
  mutate(opposition = if_else(team == "hawks", "somerset", "hawks")) %>%
  left_join(match4_bat %>% rename(op_rat = rating, opposition = team), by = "opposition") %>%
  mutate(batdelt = rating - op_rat)

match4_bowl = match4_bowl %>% 
  mutate(opposition = if_else(team == "hawks", "somerset", "hawks")) %>%
  left_join(match4_bowl %>% rename(op_rat = rating, opposition = team), by = "opposition") %>%
  mutate(bowldelt = rating - op_rat)

# Summarise match4 and predict
match4_sum = left_join(match4_bat, match4_bowl, by = "team") %>%
  select(team, batdelt, bowldelt)

pred1 <- predict(fin_rf2, new_data = match4_sum)
pred2 = predict(final_lgbm, new_data = match4_sum)

# Combine the prediction with the summary data
match4_sum_rat = match4_sum %>% bind_cols( pred1) %>%
                                      bind_cols()

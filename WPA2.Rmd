...
ggplot(match_bat_best, aes(x = bbi)) + geom_histogram(binwidth = 1)
...
match_bat_best <- match_play %>%
  ...
  group_by(team, file) %>%
  mutate(batrank = rank(desc(tot_bat_wp))) %>%
  filter(file == "1150554.json") 

ggplot(match_bat_best, aes(x = player, y = tot_bat_wp)) + geom_col() + coord_flip()
...
train_dat_in = train_dat_prob %>% mutate(roundbat1 = round(opbat_1, 2)) %>%
                                    mutate(winf2 = if_else(winf == "Yes", 1,0)) %>%
                                    group_by(roundbat1) %>%
                                      summarise(meanwin = mean(winf2), tot = n())


ggplot(train_dat_in, aes(x = roundbat1, y = meanwin, size = tot)) + geom_point()


ggplot(train_dat_prob, aes(x = bowl_1, y = OPbowl_1, col = winf)) + geom_point(alpha = 0.05)
...
bat_1_dist = train_dat_prob %>% select(bat_1, bat_2, bat_3, bat_4) %>% 
                                    pivot_longer(cols = 1:2, names_to = "meas", values_to = "player")


ggplot(bat_1_dist, aes(x = player, fill = meas)) + geom_density(alpha = 0.5)




average_par = train_dat_prob %>% 
                              select(-winf) %>%
...
preds = predict(fin_fitv4, average_par, type = "prob")



average_sum = average_par %>% bind_cols(preds)
...
max_play =  train_dat_prob %>% 
                              select(-winf) %>%
                                summarise_all(.funs = max)


min_play =  train_dat_prob %>% 
                              select(-winf) %>%
...
average_par_bat = average_par %>% sequence <- seq(from = -0.03, to = 0.35, length.out = 101)
...
bat_4 <- seq(from = -0.055, to = 0.05, length.out = 101)
...
average_par_bat = average_par %>% select(-bat_4) %>%
                                        bind_cols(param)
...
preds = predict(fin_fitv4, average_par_bat, type = "prob")



average_sum_bat1 = average_par_bat %>% bind_cols(preds)


ggplot(average_sum_bat1, aes(x = bat_4, y= .pred_Yes)) + geom_point()
...
cols = c("bat_1", "bat_2", "bat_3","bat_4","bat_5","bat_6","bat_7", "bowl_1", "bowl_2","bowl_3","bowl_4","bowl_5","OPbowl_1","OPbowl_2",
         "OPbowl_3","OPbowl_4","OPbowl_5", "opbat_1", "opbat_2","opbat_3","opbat_4","opbat_5","opbat_6","opbat_7")

sim_dat = tibble(cols) %>%
                    mutate(values_1 = 0) %>%
                          pivot_wider(names_from = cols, values_from = values_1)


pred_sim_dat = predict(fin_fitv4,sim_dat, type = "prob")
 


sim_dat2 = sim_dat %>% bind_cols(pred_sim_dat)
...
n <- length(cols)
result <- data.frame(matrix(0, nrow = 100 * n, ncol = n))
names(result) <- cols

for (i in 1:n) {
  start_row <- (i - 1) * 100 + 1
  end_row <- i * 100
  result[start_row:end_row, i] <- seq(0, 0.1, length.out = 10)
}
...
pred_sim_dat = predict(fin_fitv4,result, type = "prob")
 


results2 = result %>% bind_cols(pred_sim_dat)




bat_1_sum = results2 %>% select(bowl_1, .pred_Yes) %>%
                                filter(bowl_1 >0)



ggplot(bat_1_sum, aes(x = bowl_1, y = .pred_Yes)) + geom_point()
...
ggplot(model_dat_v4, aes(x = bowl_5)) + geom_density()
...
derby =  c("LM Reece", "Haider Ali", "WL Madsen", "JL du Plooy", "HRC Came","MH McKiernan", "BD Guest", "ZJ Chappell", "MRJ Watt", "GLS Scrimshaw", "Zaman Akhter")
glouces = c("avplay_BA", "CDJ Dent", "Miles Hammond", "OJ Price", "JMR Taylor","BG Charlesworth", "James Bracey", "DJ Lamb", "Zafar Gohar", "TMJ Smith", "DA Payne")
...
durham = c("G Clark", "AZ Lees", "MA Jones", "OG Robinson", "AJ Turner","BA Carse", "avplay_BW", "L Trevaskis", "BA Raine", "NA Sowter", "WD Parnell")
lancs= c("PD Salt", "JC Buttler", "SJ Croft", "LWP Wells", "DJ Mitchell","LS Livingstone", "DJ Vilas", "TW Hartley", "L Wood", "TE Bailey", "RP Jones")
foxes = c("NR Welch", "LJ Hill", "RK Patel", "CN Ackermann", "PWA Mulder","LPJ Kimber", "PSP Handscomb", "HJ Swindells", "CF Parkinson", "TAR Scriven", "MET Salisbury")
sussex = c("TGR Clark", "RS Bopara", "HD Ward", "Shadab Khan", "MGK Burgess","JM Coles", "N McAndrew", "FJ Hudson-Prentice", "D Ibrahim", "TS Mills", "HT Crocombe")
glamor = c("CA Ingram", "SA Northeast", "KS Carlson", "CD Fletcher", "CB Cooke","Zain-ul-Hassan", "WT Smale", "P Sisodiya", "RAJ Smith", "P Hatzoglou", "JP McIlroy")
somers = c("T Banton", "WCF Smeed", "T Kohler-Cadmore", "TB Abell", "SR Dickson","L Gregory", "BGF Green", "RE van der Merwe", "JH Davey", "C Overton", "MJ Henry")
essex = c("RJ Das", "FIN Khushi", "MS Pepper", "PI Walter", "JS Rymell","DR Sams", "WEL Buttleman", "SR Harmer", "MJJ Critchley", "S Snater", "SJ Cook")
kent = c("TS Muyeye", "DJ Bell-Drummond", "JL Denly", "SW Billings", "JM Cox","JA Leaning", "GF Linde", "G Stewart", "FJ Klaassen", "WA Agar", "MG Hogan")
surrey = c("WG Jacks", "LJ Evans", "SM Curran", "Tk Curran", "SP Narine","JL Smith", "SA Abbott", "J Overton", "CJ Jordan", "CT Steel", "TE Lawes")
northam = c("EN Gay", "CA Lynn", "JJ Cobb", "DJ Willey", "SA Zaib","LD McManus", "TAI Taylor", "avplay_BW", "AJ Tye", "BW Sanderson", "FJ Heldreich")
...
notts = c("JM Clarke", "AD Hales", "LW James", "M Montgomery", "TJ Moores","CG Harrison", "SJ Mullaney", "Imad Wasim", "Shaheen Shah Afridi", "OP Stone", "JT Ball")
bears = c("RM Yates", "AL Davies", "GJ Maxwell", "JG Bethell", "CG Benjamin","DR Mousley", "EG Barnard", "DR Briggs", "Hasan Ali", "JB Lintott", "OJ Hannon-Dalby")
hamps = c("BR McDermott", "JM Vince", "CP Wood", "JJ Weatherley", "RA Whiteley","JK Fuller", "TE Albert", "LA Dawson", "NT Ellis", "BAC Howell", "avplay_BW")
...
match_sum = team_bat(durham,bears, "durham")  %>%
                     bind_rows(team_bat(lancs, northam, "lancs")) %>%
                               bind_rows(team_bat(surrey, essex, "surrey")) %>%
                            bind_rows(team_bat(derby, worces, "derby")) %>%
                              bind_rows(team_bat(hamps, glouces, "hamps")) %>%
                                  bind_rows(team_bat(notts, foxes, "foxes")) %>%
                                    bind_rows(team_bat(somers, kent, "kent"))  %>%
                                  bind_rows(team_bat(glamor, Middlesex, "glamor")) 
...
match_1706 = team_bat(glouces, kent, "glouces")
...
mat1_sum = bat_comp(Middlesex, essex)


  cols = c(team1 = "#06d6a0", team2 = "#ffd166")
  
  ggplot(mat1_sum, aes(x = coltit, y = tot_bat_wp, col = team, label = player)) + 
                                                                      geom_point(size = 5) +
...
player_comp = function(x,y) {
 ...
 mat1_sum = player_comp(royals, kings)
 
  cols = c(team1 = "#06d6a0", team2 = "#ffd166")
  
  ggplot(mat1_sum, aes(x = rank, y = val, col = team)) + 
                                                                      geom_point(size = 5) +
...
pheonix = c("Miles Hammond", "WCF Smeed", "DR Mousley", "BM Duckett", "BAC Howell", "LS Livingstone", "JG Bethell", "Shadab Khan", "CG Benjamin", "JL Smith","TG Helm", "HJH Brookes", "AF Milne", "KW Richardson", "MM Ali", "CR Woakes")
spirit = c("Z Crawley", "DW Lawrence", "DJ Bell-Drummond", "LA Dawson", "RS Bopara", "JA Thompson", "DJ Mitchell", "AM Rossington", "MS Pepper", "MS Wade", "CP Wood", "NT Ellis", "DJ Worrall", "MS Crane")
origionals = c("WL Madsen", "PI Walter", "LJ Evans", "MDE Holden", "TA Lammonby", "AJ Turner", "BA Raine", "Usama Mir", "PD Salt", "JC Buttler", "RJ Gleeson", "MT Stanley", "J Little", "Zaman Khan", "TW Hartley")
superch = c("A Lyth","AJ Hose", "HC Brook", "DJM Short", "SA Zaib", "BA Stokes", "D Wiese", "BFW de Leede", "T Banton", "OG Robinson", "WD Parnell", "MJ Potts", "BA Carse", "RJW Topley", "AU Rashid", "CF Parkinson")
invinc = c("WG Jacks", "JJ Roy", "JM Cox", "TS Muyeye", "RA Whiteley", "SM Curran", "TK Curran", "SP Narine", "TE Lawes", "JDS Neesham", 
           "SW Billings", "H Klaassen", "AAP Atkinson", "ZJ Chappell", "Ihsanullah", "DR Briggs", "NA Sowter", "A Zampa")
brave =c("JJ Weatherley", "JM Vince", "TH David", "DP Conway", "FH Allen", "JL du Plooy", "GHS Garton", "JK Fuller", "MJ Santner", "AL Davies", "C Overton", "CJ Jordan", "TS Mills", "MD Fisher", "Rehan Ahmed", "JA Chohan")
rocket = c("AD Hales", "DJ Malan", "C Munro", "SR Hain", "SR Patel", "L Gregory", "TJ Moores", "T Kohler-Cadmore", "L Wood", "SJ Cook", "DR Sams", "JA Turner", "BTJ Wheal", "M Carter", "IS Sodhi")
fire = c("GD Phillips", "SS Eskinazi", "TB Abell", "DA Douthwaite", "BGF Green", "RE van der Merwe", "LWP Wells", "DJ Willey", "JM Clarke", "CB Cooke", "DA Payne", "GLS Scrimshaw", "JT Ball", "Haris Rauf", "JM Bairstow")
...
squads_ph = bat_sum %>% bind_cols(bowl_sum) %>%
                            mutate(team = y )
...
squad_sum = the100_s(pheonix, "pheonix") %>% 
                  bind_rows(the100_s(spirit, "spirit")) %>% 
                    bind_rows(the100_s(origionals, "origionals")) %>%
                          bind_rows(the100_s(superch, "northern")) %>% 
                    bind_rows(the100_s(invinc, "invinc")) %>%
                             bind_rows(the100_s(brave, "brave")) %>%
                          bind_rows(the100_s(rocket, "trent")) %>% 
                    bind_rows(the100_s(fire, "fire")) 
...
squad_sumop = the100_s2(pheonix, "pheonix") %>% 
                  bind_rows(the100_s2(spirit, "spirit")) %>% 
                    bind_rows(the100_s2(origionals, "origionals")) %>%
                          bind_rows(the100_s2(superch, "northern")) %>% 
                    bind_rows(the100_s2(invinc, "invinc")) %>%
                             bind_rows(the100_s2(brave, "brave")) %>%
                          bind_rows(the100_s2(rocket, "trent")) %>% 
                    bind_rows(the100_s2(fire, "fire")) 
                        ...
                        bat_sum = squads %>%
                              ...
                              mutate(batrank = rank(desc(tot_bat_wp)))  %>%
                                  filter(batrank < 8) %>%
                                    ...
                                    select(player, batrank, bf, tot_bat_wp) %>%
                                      mutate(coltit = if_else(batrank == 1, "bat_1",
                                                              if_else(batrank == 2, "bat_2",
                                                                if_else(batrank == 3, "bat_3",
                                                                  if_else(batrank == 4, "bat_4",
...
squads_ph = bat_sum %>% bind_rows(bowl_sum) %>%
                            mutate(team = y )
...
squad_sum_play = the100_s3(pheonix, "pheonix") %>% 
                  bind_rows(the100_s3(spirit, "spirit")) %>% 
                    bind_rows(the100_s3(origionals, "origionals")) %>%
                          bind_rows(the100_s3(superch, "northern")) %>% 
                    bind_rows(the100_s3(invinc, "invinc")) %>%
                             bind_rows(the100_s3(brave, "brave")) %>%
                          bind_rows(the100_s3(rocket, "trent")) %>% 
                    bind_rows(the100_s3(fire, "fire")) 
...
bat_sum_team =squad_sum_play %>% filter(!is.na(batrank))




ggplot(bat_sum_team, aes(x = team, y = tot_bat_wp, size = bf, col = team)) + geom_jitter() 



bowl_sum_team = squad_sum_play %>% filter(!is.na(bowlr))



ggplot(bowl_sum_team, aes(x = team, y = wpa6, size = bb, col = team)) + geom_jitter() 
...
teams = c("pheonix", "spirit", "origionals", "northern", "invinc", "brave", "trent", "fire")



hun_team = tibble(no, teams)

match_sum = squad_sum %>% mutate(match = 8) %>% 
                                  uncount(match) %>% 
                                    group_by(team) %>% 
                                        mutate(no = 1:n()) %>% 
                                          left_join(hun_team, by = "no") %>%
                                      left_join(squad_sumop, by = "teams")





match_pred = predict(fin_fitv4, match_sum, type = "prob")


match_sum2 = match_sum %>% bind_cols(match_pred)  
...
team = c("pheonix", "spirit", "origionals", "northern", "invinc", "brave", "trent", "fire")
rival = c("trent", "invinc", "northern", "origionals", "spirit", "fire", "pheonix", "brave")

riv_time = tibble(team, rival)

tour_tab = squad_sum %>% mutate(match = 8) %>% 
                                  uncount(match) %>% 
                                    group_by(team) %>% 
                                        mutate(no = 1:n()) %>% 
                                          left_join(hun_team, by = "no")  %>%
                                              left_join(riv_time, by = "team") %>%
                                                  mutate(team_mes = if_else(team == teams, 1,0)) %>%
                                                    mutate(team3 = if_else(team_mes == 1, rival, teams)) %>%
                                            select(-teams) %>%
                                                rename(teams = team3) %>%
                                      left_join(squad_sumop, by = "teams")
...
match_pred = predict(fin_fitv4, tour_tab, type = "prob")


tour_tab2 = tour_tab %>% bind_cols(match_pred) %>%
                              mutate(points = if_else(.pred_Yes >0.5, 2,0), delta = .pred_Yes - .pred_No) %>%
                                group_by(team) %>%
                                    summarise(totpoint = sum(points), meandel = mean(delta))
...
match_sum = team_bat(rocket,brave, "rocket")  %>%
                     bind_rows(team_bat(fire, origionals, "fire")) %>%
                               bind_rows(team_bat(spirit, invinc, "spirit")) %>%
                            bind_rows(team_bat(superch, worces, "derby")) %>%
                              bind_rows(team_bat(hamps, glouces, "hamps")) %>%
                                  bind_rows(team_bat(notts, foxes, "foxes")) %>%
                                    bind_rows(team_bat(somers, kent, "kent"))  %>%
                                  bind_rows(team_bat(glamor, Middlesex, "glamor")) 
...
royals = c("R Powell", "JO Holder", "KR Mayers", "Qais Ahmad", "LJ Evans", "A Athanaze", "OC McCoy", "RE van der Merwe", "ROO Jordan", 
           "RRS Cornwall", "D Ferreira", "JP Greaves", "J Bishop", "NRJ Young", "RA Clarke", "RR Simmonds")
warriers = c("Imran Tahir", "SO Hetmyer", "Mohammad Haris", "OF Smith", "R Shepherd", "Azam Khan", "SD Hope", "Saim Ayub", "G Motie", "D Pretorius", "K Sinclair", "KMA Paul", "C Hemraj", "RR Beaton", "JA Sinclair")
tallawahs = c("Imad Wasim", "BA King", "FA Allen", "Mohammad Amir", "Naveen-ul-Haq", "CJ Green", "BCJ Cutting", "J Blackwood", "SSJ Brooks", "HR Walsh", "RA Reifer", "AA Jangoo", "SR Taylor", "SK Springer", "NA Gordon", "KSA McKenzie", "JM James")
patriots = c("E Lewis", "T Stubbs", "ADS Fletcher", "SE Rutherford", "DC Drakes", "SS Cottrell", "GF Linde", "Y Cariah", "O Thomas", "C Bosch", "D Brevis", "JU Goolie", "Izharulhaq Naveed", "KHM James", "J Da Silva", "AR Nedd", "AT Rayudu")
kings = c("F du Plessis", "J Charles", "MD Shanaka", "AS Joseph", "RL Chase", "Jair McAllister", "Sikandar Raza", "P Hatzoglou", "PBB Rajapaksa", "RS Primus", "J Royal", "Sadrack Descarte", "K Pierre", "LJA Julien","MW Forde", "CB Sole", "SC Williams")
knight = c("KA Pollard", "AD Russell", "SP Narine", "N Pooran", "RR Rossouw", "AJ Hosein", "DJ Bravo", "MJ Guptill", "Noor Ahmad", 
           "M Pathirana", "JNT Seales", "M Deyal", "CAK Walton", "T Hinds", "K Alleyne", "JC Carmichael")
...
allplay_stats = play_stats2 %>%  
                        
                            left_join(player_list, by = "player")



average_bowl = allplay_stats %>%
                              filter(bb > 100) %>%
                                    group_by(comp2) %>% 
                    summarise(meanwpa6 = mean(wpa6), mbpi = mean(bpi)) %>%
                      mutate(player = "avplay_BW") %>%
...
average_bat = allplay_stats %>%
                              filter(bf > 100) %>%
                                group_by(comp2) %>% 
                    summarise(meanwpa6 = mean(wpa10), meanball= mean(bbi)) %>%
                      mutate(player = "avplay_BA") %>%
                            rename(wpa10_av = meanwpa6, bbi_av = meanball) %>%
              mutate(bf = 0, totwap = -0.2, inn = 0, tot_bat_wp_av = wpa10*bbi_av, bb = 0, tot_wpa = -5, match = 0, bpi = 0 ) %>%
                              select(comp2, wpa10_av, bbi_av)





caribean_bat = allplay_stats %>% 
                                    select(comp2, player, wpa10, bbi, bf) %>%  
                                        mutate(tot_bat_wp = wpa10/10 * bbi) %>% 
                                     
                                          filter(comp2 == "Caribbean Premier League")



caribean_bowl = allplay_stats %>% select(comp2, player, wpa6, bb, bpi) %>% 
                                          filter(comp2 == "Caribbean Premier League")




carribean_bat2 = caribean_bat %>% left_join(average_bat, by = "comp2") %>%
                                        mutate(batdel = 100 - bf) %>%
                                          mutate(adj = wpa10_av/10 * batdel) %>%
                                            mutate(act = wpa10/10*bf) %>%
                                                mutate(wpa_new = if_else(batdel >0 , (act + adj)/10, wpa10)) %>%
                                                  select(-tot_bat_wp) %>%
                                                    mutate(tot_bat_wp = wpa_new/10*bbi) 
...
cpl_squad1 = cp_squad(royals, "royals") %>%
                          bind_rows(cp_squad(warriers, "warriers")) %>%
                            bind_rows(cp_squad(tallawahs, "tallawahs")) %>%
                              bind_rows(cp_squad(patriots, "patriots")) %>%
                            bind_rows(cp_squad(kings, "kings")) %>%
                                bind_rows(cp_squad(knight, "knight")) %>% 
                                  left_join(player_list, by = "player") %>%
                                    left_join(carribean_bat2, by = "player") %>%
                                        left_join(caribean_bowl, by = "player") 
...
mod_sum_bat = cpl_squad1 %>% group_by(team) %>%
                                   mutate(batrank = rank(desc(tot_bat_wp)))  %>%
                                  filter(batrank < 8) %>%
...
mod_sum_bowl = cpl_squad1 %>% group_by(team) %>%
                                 mutate(bowlrank = rank(desc(bpi)))  %>%
                                filter(bowlrank < 6) %>%
                                  mutate(bowlr = rank(desc(wpa6))) %>%
                                    select(team, bowlr, wpa6) %>%
                                      mutate(coltit = if_else(bowlr == 1, "bowl_1",
                                                              if_else(bowlr == 2, "bowl_2",
                                                                if_else(bowlr == 3, "bowl_3",
                                                                  if_else(bowlr == 4, "bowl_4",
...
all_teams = mod_sum_bat %>% left_join(mod_sum_bowl, by = "team") 
...
cpl_op = mod_sum_bat %>% left_join(mod_sum_bowl, by = "team") %>% 
                                  rename(opteam = team )
...
opteam = c("royals", "warriers", "kings", "knight", "tallawahs", "patriots")



cpl_team = tibble(no, opteam)


all_teams2 = all_teams %>% mutate(no2 = 6) %>%
                              uncount(no2) %>%
                                group_by(team) %>%
                                  mutate(no = 1:n()) %>%
                                    left_join(cpl_team, by = "no") %>%
                                      left_join(cpl_op, by = "opteam")




cpl_pred = predict(fin_fitv4, all_teams2, type = "prob")


cpl_sum2 = all_teams2 %>% bind_cols(cpl_pred)  
...
mod_sum_bat = cpl_squad1 %>% group_by(team) %>%
                                  ...
                                  filter(batrank < 8) %>%
                                    ...
                                    select(team, player, batrank, tot_bat_wp, bf) %>%
                                      mutate(coltit = if_else(batrank == 1, "bat_1",
                                                              if_else(batrank == 2, "bat_2",
                                                                if_else(batrank == 3, "bat_3",
                                                                  if_else(batrank == 4, "bat_4",
...
bat_sum = mod_sum_bat %>% filter(team %in% c("royals", "tallawahs"))



ggplot(bat_sum, aes(x = batrank, y = tot_bat_wp, col = team, size = bf)) + geom_point()
...
mod_sum_bowl = cpl_squad1 %>% group_by(team) %>%
                                ...
                                filter(bowlrank < 6) %>%
                                  mutate(bowlr = rank(desc(wpa6))) %>%
                                    select(team, player, bowlr, wpa6, bb) %>%
                                      mutate(coltit = if_else(bowlr == 1, "bowl_1",
                                                              if_else(bowlr == 2, "bowl_2",
                                                                if_else(bowlr == 3, "bowl_3",
                                                                  if_else(bowlr == 4, "bowl_4",
...
bowl_sum = mod_sum_bowl %>% filter(team %in% c("royals", "k"))



ggplot(bowl_sum, aes(x = bowlr, y = wpa6, col = team, size = bb)) + geom_point()
...
test = fin_dat %>% filter(comp == "Vitality Blast") %>%
                                filter(yy == 2023)
...
ipl_stats = play_stats2 %>%  
                         filter(comp2 == "Indian Premier League") %>%
                            mutate(bb2 = if_else(is.na(bb), 0, bb)) %>%
                          mutate(totball = bf + bb2) %>%
                            mutate(batper = bf/totball, bowlper = bb/totball) %>%
                                mutate(position = if_else(batper > .7, "Batter", 
                                                          if_else(bowlper > 0.6, "Bowler", "All Rounder")
                                                        ))
 

average_bowl_ipl = ipl_stats %>%
                              filter(bb > 100) %>%
                    summarise(meanwpa6 = mean(wpa6), mbpi = mean(bpi)) %>%
                      mutate(player = "avplay_BW") %>%
                            rename(wpa6 = meanwpa6, bpi = mbpi) %>%
...
ipl_statsv2 = ipl_stats %>% bind_rows(average_bowl_ipl) %>%
                                        bind_rows(average_bat_ipl)
...
av_bat2_ipl = average_bat_ipl %>% select(wpa10) %>%
                            rename(av_play = wpa10)
...
ipl_statsv3 = ipl_statsv2 %>% left_join(batt_u100, by = "player") %>%
                  mutate(act_tot_wpa = if_else(is.na(tot_wpa_adj), tot_bat_wp, tot_wpa_adj)) %>%
                          select(-tot_bat_wp) %>%
                                  rename(tot_bat_wp = act_tot_wpa)
...
av_bowl2 = average_bowl_ipl %>% select(wpa6) %>%
                            rename(av_play = wpa6)
...
ipl_statsv4 = ipl_statsv3 %>% left_join(bowl_u100, by = "player") %>%
                  mutate(act_tot_wpa = if_else(is.na(wpa_sum_adj), wpa6, wpa_sum_adj)) %>%
                          select(-wpa6) %>%
                                  rename(wpa6 = act_tot_wpa) %>%
                              select(comp2.x, player, bf, totwap, wpa10, bbi, bb, tot_wpa, bpi, totball, position, tot_bat_wp, wpa6)

...
n <- length(cols)
result <- data.frame(matrix(0, nrow = 10 * n, ncol = n))
names(result) <- cols

for (i in 1:n) {
  start_row <- (i - 1) * 10 + 1
  end_row <- i * 10
  result[start_row:end_row, i] <- seq(0, 0.06, length.out = 10)
}
...
fire = c("GD Phillips", "SS Eskinazi", "TB Abell", "DA Douthwaite", "BGF Green", "RE van der Merwe", "LWP Wells", "DJ Willey", "JM Clarke", "CB Cooke", "DA Payne", "GLS Scrimshaw", "JT Ball", "Haris Rauf", "Shaheen Shah Afridi")

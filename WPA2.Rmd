...
model_test = model_dat1 %>% mutate(winno = if_else(win == "Yes", 1,0)) %>%
                                mutate(roundbat = round(batdelt,0)) %>%
                                      group_by(roundbat) %>%
...
boundary_zoom = boundary %>% filter(batdelt > -0.01) %>%
                                filter(batdelt < 0.01)
...
ggplot(boundary_zoom, aes(x = batdelt, y = bowldelt, col = .pred_Yes)) + geom_point()
...
match4_sum_rat = bind_cols(match4_sum, pred1)

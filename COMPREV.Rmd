...
ipl_odds = read_csv("iplmatch.csv")
...
team_sum5 = team_sum4 %>% left_join(ipl_odds, by = c("file", "team.x", "opteam")) %>%
                                  mutate(odd_per_teamx = 1/team.x_win, odd_per_opteam = 1/opteam_win) %>%
...
ipl_odds2 = ipl_odds %>% select(team.x, opteam, file) %>%
                              pivot_longer(cols = 1:2, names_to = "team_type", values_to = "team")



ipl_odds3 = ipl_odds %>% select(team.x_win, opteam_win, file) %>%
...
bet_win_sum = team_sum5 %>% mutate(predel = .pred_Yes - .pred_No)  %>%
                                  mutate(pred_win = if_else(predel > 0, team.x, opteam))  %>%
                                      left_join(ipl_odds_sum, by = c("file", "pred_win")) %>%
                                        mutate(winnings = if_else(pred_win == winning_team, odds*5, -5)) %>%
                                          ungroup() %>%
                                            select(team.x, opteam, .pred_No, .pred_Yes, Match_dat.x, team.x_win, 
                                                   opteam_win, pred_win, winning_team, odds, winnings)
...
bet_win_sum_del = team_sum5 %>% 
                            mutate(predel = .pred_Yes - .pred_No)  %>%
                                  mutate(pred_win = if_else(predel > 0, team.x, opteam))  %>%
                                      left_join(ipl_odds_sum, by = c("file", "pred_win")) %>%
                            select(team.x, opteam, .pred_No, .pred_Yes, Match_dat.x, winning_team, team.x_win, opteam_win, act_teamx, act_opteam, predel, odds) %>%
                              mutate(pred_win = if_else(predel > 0, team.x, opteam)) %>%
